...
/**
 * Game screen for playing mini-adventures.
 * Handles keyboard input, game state display, and win/lose conditions.
 */
public class GameScreen extends JPanel {
    ...
    // UI Components
    private RealmMapPanel mapPanel;
    ...
    public GameScreen(GMAEGUI mainGUI) {
    ...
    private void initializeComponents() {
        // Map panel for displaying the realm
        mapPanel = new RealmMapPanel(null);
    ...
    private void handleKeyPress(KeyEvent e) {
        ...
        if (success) {
            ...
            if (currentAdventure instanceof RelicHuntAdventure) {
                RelicHuntGame game = ((RelicHuntAdventure) currentAdventure).game;
                int loseResult = game.isLost();
                if (loseResult > 0) {
                    handleGameLost(loseResult);
                    return;
                }
            }
    ...
    private void handleGameLost(int playerNum) {
        gameActive = false;
        
        User loser = playerNum == 1 ? gameManager.getPlayer1() : gameManager.getPlayer2();
        String message = loser.getUsername() + " has fallen!\n\nGame Over!";
        String title = "Defeat!";
        ...
        gameInfoLabel.setText("Game Over! Press Reset to try again or Back to menu.");
    }
    ...
    public void startAdventure(MiniAdventure adventure, User player1, User player2, boolean competitive) {
        ...
        if (success) {
            ...
            // Update map panel
            mapPanel.setRealmSpace(adventure.getRealm());
    ...
    private void updateDisplay() {
        ...
        if (adventureState != null) {
            ...
            // Update game info with health
            if (gameActive) {
                int relicsCollected = (Integer) adventureState.get("relicsCollected");
                int totalRelics = (Integer) adventureState.get("totalRelics");
                int player1Health = (Integer) adventureState.get("player1Health");
                int player2Health = (Integer) adventureState.get("player2Health");
                int maxHealth = (Integer) adventureState.get("maxHealth");
                
                gameInfoLabel.setText("Relics: " + relicsCollected + "/" + totalRelics + 
                    " | P1 Health: " + player1Health + "/" + maxHealth + 
                    " | P2 Health: " + player2Health + "/" + maxHealth +
                    " | Mode: " + (gameManager.isCompetitive() ? "Competitive" : "Co-op"));
            }
